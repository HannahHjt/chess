...
#bord info :  "lb" represents the left black rook,
# "hb" represents the left black knight,
# "cb" represents the left black bishop, 
# "qb" represents the black queen, 
# "kb" represents the black king,
# "pb" represents the black pawns, 
# "dot" represents empty squares,
# "pw" represents the white pawns,
# "lw" represents the left white rook,
# "hw" represents the left white knight,
# "cw" represents the left white bishop,
# "qw" represents the white queen,
# "kw" represents the white king.
class Board:
    def __init__(self):
        ...
        self.reboard = [copy_board]             #self.reboard represents a history of boards for undo/redo
    ...
    #checks if a move from position (y, x) to position (y1, x1) is valid
    ...
    #checks if any of the opponent's pieces can beat the piece at position (y, x)
        ...
        for v, i in enumerate(t.board):
            for k, j in enumerate(i):
                if t.cheker(v, k, y, x):
                    winc_can = True
        return winc_can
    #This method finds the first opponent's piece that can beat the piece at position (y, x)
    def EZFV(self, y, x):
        for v, i in enumerate(t.board):
            for k, j in enumerate(i):
                if t.cheker(v, k, y, x):
    ...
    #checks if the piece at position (y, x) can move to the position (v, k) based on cheker() and can_move()
    def echis(self, y, x):
        moves = []
        for v, i in enumerate(t.board):
            for k, j in enumerate(i):
    ...
    #checks if you can make a valid move considering threats
    def can_move(self, y, x, y1, x1):
        q = kw if t.board[y][x].colour == 1 else kb
        if t.board[y][x].__class__ != King:    #checks if (y, x) is not King
            ...
            t.board[y1][x1] = save
            return winc
    #checks if there are any winning positions
    def mat(self):
        winc_mat1 = True   ## Variable to track if there is a winning position
        winc_mat2 = []     #store the winning positions
        for v, i in enumerate(t.board):
            for k, j in enumerate(i):
                if t.cheker(v, k, kw.y, kw.x) or t.cheker(v, k, kb.y, kb.x):
                    q = kw if t.board[v][k].colour == 0 else kb   # Determine which player's piece is threatening
                    # Check if the current piece at position (v, k) can beat the opponent's king and cannot move away
                    if self.beatofcan(v, k) and not self.can_move(self.EZFV(v, k)[0], self.EZFV(v, k)[1], v, k):
                        winc_mat1 = True
                    else:
                        winc_mat1 = False
                    # Check all adjacent positions around the current threatening piece
                    for i in range(-1, 2):
                        for j in range(-1, 2):
                            try:
                                ...
                                winc_mat2.append(False)
                                continue
        if True in winc_mat2:   #least one winning position
            winc_mat2 = True  
        else:        #no winning position
            winc_mat2 = False
        return winc_mat1 + winc_mat2       # Return the overall result as a sum of winc_mat1 and winc_mat2
